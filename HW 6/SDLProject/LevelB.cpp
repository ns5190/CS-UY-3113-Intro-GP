...
unsigned int LEVEL_B_BACKGROUND_2_DATA[] =
{
...
void LevelB::initialise()
{
    ...
    m_font_B = Utility::load_texture("font.png");
    
    GLuint map_texture_id = Utility::load_texture("tileset.png");
    m_game_state.map = new Map(LEVEL_WIDTH, LEVEL_HEIGHT, LEVEL_B_DATA, map_texture_id, 1.0f, 25, 79);
    
    m_game_state.background_map_1 = new Map(LEVEL_WIDTH, LEVEL_HEIGHT, LEVEL_B_BACKGROUND_1_DATA, map_texture_id, 1.0f, 25, 79);
    m_game_state.background_map_2 = new Map(LEVEL_WIDTH, LEVEL_HEIGHT, LEVEL_B_BACKGROUND_2_DATA, map_texture_id, 1.0f, 25, 79);
    ...
    m_game_state.player->set_position(glm::vec3(12.5f, -9.0f, 0.0f));
    ...
    for (int i = 0; i < ANIMAL_COUNT; i++) {
        ...
        m_game_state.animals[i].set_ai_type(CHICKEN);
        m_game_state.animals[i].set_ai_state(WALKING);
        m_game_state.animals[i].m_texture_id = Utility::load_texture("chicken.png");
        
        m_game_state.animals[i].m_walking[m_game_state.animals[0].LEFT]     = new int[4] { 12, 13, 14, 15 };
        ...
        m_game_state.animals[i].m_animation_rows = 7;
        m_game_state.animals[i].set_height(1.0f);
        m_game_state.animals[i].set_width(1.0f);
        m_game_state.animals[i].set_size(glm::vec3(0.5f, 0.5f, 0.0f));
    }
    
    m_game_state.item = new Entity[EGG_COUNT];
    
    for (int i = 0; i < EGG_COUNT; i++) {
        m_game_state.item[i].set_entity_type(ITEM);
        m_game_state.item[i].m_texture_id = Utility::load_texture("egg.png");
        m_game_state.item[i].set_height(1.0f);
        m_game_state.item[i].set_width(1.0f);
        m_game_state.item[i].set_size(glm::vec3(1.2f, 1.2f, 0.0f));
...
void LevelB::update(float delta_time)
{
    ...
    m_game_state.player->update(delta_time, m_game_state.player, m_game_state.item, EGG_COUNT, m_game_state.map);
    ...
    for (int i = 0; i < ANIMAL_COUNT; i++) {
        ...
        if (m_game_state.animals[i].m_can_lay && !m_game_state.animals[i].m_laid_egg) { // true and false
            m_game_state.animals[i].m_laid_egg = true;
            m_game_state.item[egg_index].activate();
            m_game_state.item[egg_index].set_position(m_game_state.animals[i].get_position());
            egg_index++;
        }
    }
    for (int i = 0; i < EGG_COUNT; i++) {
...
void LevelB::render(ShaderProgram *program)
{
    if (end_scene) {
        ...
        if (end_scene_timer < 2.0) {
            ...
            Utility::draw_text(program, m_font_B, std::string("Times Up!"),
        ...
        if (end_scene_timer >= 2.0) {
            ...
            Utility::draw_text(program, m_font_B, std::string("Eggs Stolen:"), 1.5f, -0.8f, glm::vec3(-3.0f, 2.5f, 0.0f));
            if (!end_scene_ping[1]) {
        ...
        if (end_scene_timer >= 4.0) {
            ...
            Utility::draw_text(program, m_font_B, total_items, 1.5f, -0.8f, glm::vec3(1.5f, 0.0f, 0.0f));
            if (!end_scene_ping[3]) {
    ...
    Utility::draw_text(program, m_font_B, std::string("Task 2: Collect the eggs"), 0.8f, -0.4f, glm::vec3(timer_x - 9.5f, timer_y + 7.0f, 0.0f)); // align with timer
    Utility::draw_text(program, m_font_B, std::string("Time Left: ") + time_left, 0.8f, -0.4f, glm::vec3(timer_x + 4.5f, timer_y + 7.0f, 0.0f));
}
